<template>
    <template v-for="product in printerData.products" :key="product.id">
        ...
        <template v-if="columns === 1">
            <div
                v-for="(num, index) in (product.count || 1)"
                :key="`${product.id}-${num}`"
                class="price-print-barcode"
                :class="{ 'last-barcode': isLastBarcode(product, index) }"
...
export default {
    ...
    computed: {
        ...
        // Обчислюємо загальну кількість штрих-кодів для визначення останнього
        totalBarcodeCount() {
            return this.printerData.products.reduce((sum, product) => {
    ...
    methods: {
        // Метод для визначення, чи є поточний штрих-код останнім
        isLastBarcode(product, index) {
            const products = this.printerData.products;
            const currentProductIndex = products.indexOf(product);
            const isLastProduct = currentProductIndex === products.length - 1;
            const isLastInProduct = index === (product.count || 1) - 1;
            
            return isLastProduct && isLastInProduct;
        },
            ...
            style.textContent = `
                ...
                @media print {
                    *:not(.price-print-barcode) {
                    ...
                    /* Останній елемент НЕ повинен мати розрив сторінки після себе */
                    .price-print-barcode:last-child,
                    .price-print-barcode.last-barcode,
                    .price-print-barcode:only-child {
...
@media print {
    ...
    /* Останній елемент або єдиний елемент не повинен мати розрив сторінки */
    .price-print-barcode:last-child,
    .price-print-barcode.last-barcode,
    .price-print-barcode:only-child {
        break-after: auto !important;
